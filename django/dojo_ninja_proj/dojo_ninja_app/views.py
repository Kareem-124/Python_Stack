...
def process(request):
    ...
    #Add A new Ninja
        ...
        ninja.objects.create(first_name = request.POST['first_name'] , last_name = request.POST['last_name'] , dojo_id = dojo_id)
    
    #Delete Dojo and there associated ninjas 
    elif request.POST['process'] == "delete_dojo":
        dojo_id = int(request.POST['dojo_name_to_delete']) # Get the id for the dojo to be deleted
        delete_dojo = dojo.objects.get(id = dojo_id) # Get the Dojo object
        for ninjas in delete_dojo.ninjas.all(): # iterate through the ninjas in the selected dojo and delete them one by one from the Data Base
            ninjas.delete()
