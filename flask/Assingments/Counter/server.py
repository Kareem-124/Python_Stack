...
def process():
    # The user entered a number to increment
    if request.form['form_type'] == "user_increment":
        # if the user didn't send numerical value
        if (request.form['user_counter']).isnumeric() == False:
            session['increment'] -= 1
            return redirect('/')
